...
CHENTRY=$(echo -e "$TOGGLE\n$LIST" | uniq -u | rofi -dmenu -selected-row 1 -config "/home/arjunpavanje/.config/rofi/launchers/type-2/style-5.rasi")
# store selected SSID
...
if [ "$CHENTRY" = "" ]; then
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if nmcli connection delete "$CHSSID" && nmcli dev wifi con "$CHSSID" password "$WIFIPASS"
		then
